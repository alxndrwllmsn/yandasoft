...
void process(const IConstDataSource &ds, const LOFAR::ParameterSet &parset) {
  ...
  for (IConstDataSharedIter it=ds.createConstIterator(sel,conv);it!=it.end();++it) {  
       ...
       if (replaceFlags) {
           buf += flagOutliers(padSecond(replaceFlagsWithZeros(casacore::transpose(it->visibility().yzPlane(polIndex)), casacore::transpose(it->flag().yzPlane(polIndex))),padding));
       } else {
           buf += flagOutliers(padSecond(casacore::transpose(it->visibility().yzPlane(polIndex)),padding));
       }
       avgTime += it->time();
       if (++counter == nAvg) {
           buf /= float(nAvg);
  ...
  if ((currentStep>0) || (counter!=0)) {
      const casa::uInt row2export = parset.getUint32("row2export",0u); // choose row to export 
