...
def analyseResult(spr):
   ...
   if abs(stats['rms']-stats['peak'])>0.3 or abs(stats['rms']-stats['median'])>0.3 or abs(stats['peak']-stats['median'])>0.3:
...
def runTestsSelfcal(solverType):
    """
    Runs selfcal tests for a given calibration solver type.
    """
    spr = SynthesisProgramRunner(template_parset = 'selfcaltest_template.in')
    ...
    spr.addToParset("Csimulator.calibaccess.parset = selfcalgains.in")
    ...
    res_gains = loadParset("result.dat")
    orig_gains = loadParset("selfcalgains.in")
    for k,v in list(res_gains.items()):
       ...
       if abs(abs(v)-abs(orig_val))>0.04:
          raise RuntimeError(f"Gain parameter {k} has a value of {v} which differs by {abs(v-orig_val):.5f}, ({abs(v)-abs(orig_val):.5f} in absolute value) from model value {orig_val}")

    # # now try to obtain time-dependent solution (note, a proper analysis of the result is not done)
    print("Third run of selfcal. Time-dependent solution for antennagains")
    spr.initParset()
    spr.addToParset("Cimager.selfcal.solver = " + solverType)
...
def generateGains(file, gainrms, phaserms, nant = 36):
    # set a seed to make it repeatable
    np.random.seed(42)
    with open(file,'w') as f:
        gains = np.random.normal(1.0,gainrms,nant*2)
        phases = np.random.normal(0.0,phaserms,nant*2)
        # leave first antenna phases at 0
        phases[0:2] =0
        # leave mean gain unchanged since selfcal can't fix that
        for k in [0,1]:
            scale=0
            n=0
            for i in range(0,nant-1):
                for j in range(i+1,nant):
                    scale+=gains[k+2*i]*gains[k+2*j]
                    n+=1
            scale/=n
            gains[k:nant*2:2]/=np.sqrt(scale)
        cgains = gains*np.exp((0+1j)*phases)
        for ant in range(nant):
            f.write(f"gain.g11.{ant}.0 = [{cgains[2*ant].real:.5f},{cgains[2*ant].imag:.5f}]\n")
            f.write(f"gain.g22.{ant}.0 = [{cgains[2*ant+1].real:.5f},{cgains[2*ant+1].imag:.5f}]\n")
#==================================================================================================
if __name__ == '__main__':
    ...
    generateGains("selfcalgains.in",0.2,1.0)
    runTestsSelfcal("SVD")
