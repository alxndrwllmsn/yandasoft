...
void CompositeUVWeightCalculator::add(const boost::shared_ptr<IUVWeightCalculator> &task)
{
...
void CompositeUVWeightCalculator::process(casacore::Matrix<float> &wt) const
{
   for (std::vector<boost::shared_ptr<IUVWeightCalculator> >::const_iterator ci = itsTasks.begin(); ci != itsTasks.end(); ++ci) {
        ASKAPDEBUGASSERT(*ci);
        (*ci)->process(wt);
   }
}
